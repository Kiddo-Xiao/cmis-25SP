...
QUESTION_TEXT_SIZE = 16
# Define different lods
LOD_TEXT_SIZE = {0: 12, 1: 14, 2: 14}
LOD_BG_COLOR = {0: "#F0FFF0", 1: "#DFFFD6", 2: "#90EE90"}
...
class MainAppUI:
    def __init__(self, parent, app, lod, placement, ui):
        ...
        ''' Optimal-2: Define font and color based on LoD '''
        # Define font settings based on LoD
        font_sizes = LOD_TEXT_SIZE
        font_style = ("Arial", font_sizes.get(lod, 12), "normal")
        bg_color = (LOD_BG_COLOR.get(lod))
        # Create label with LoD-specific font
        self.label = tk.Label(self.parent, 
                            text=f"{self.app.name}:\n{self.app.get_lod(self.lod)}", 
                            font=font_style, 
                            wraplength=wrap, 
                            borderwidth=1, 
                            relief="solid", 
    ...
    def update_lod(self):
        ...
        ''' Optimal-1: Fix size regardless of Lod when at rightmost column '''
        max_col = UI.COLS - 1  
        max_row = UI.ROWS - 1
        at_right_edge = (self.placement[0] + colspan - 1) >= max_col
        at_right_down = (self.placement[1] + rowspan - 1) >= max_row
        # Define font sizes based on LoD
        font_sizes = LOD_TEXT_SIZE
        # Fix font size and wrap length to match LoD 0
        if at_right_down:
            font_size = 10
            wrap = (1 * UI.BLOCK_SIZE) * 0.9
        elif at_right_edge:
            font_size = font_sizes[0] 
            wrap = (1 * UI.BLOCK_SIZE) * 0.9
        else:
            font_size = font_sizes.get(self.lod, 12)
            wrap = (colspan * UI.BLOCK_SIZE) * 0.9 

        font_style = ("Arial", font_size, "normal")
        bg_color = (LOD_BG_COLOR.get(self.lod))
...
class UI:
    ...
    def init_all_panel(self):
        ...
        ''' Optimal-4: Sorts the remaining apps by relevance score '''
        remain_apps = [app for app in self.apps.keys() if app not in self.main_apps]
        sort_apps = sorted(remain_apps, key=lambda app: self.relevance.get(app, 0), reverse=True)
        for name in sort_apps:
    ...
    ''' Optimal-3: Ensures user must enter input before submit '''
    def validate_input(self, event=None):
        if self.entry_answer.get().strip():  # Check if entry field is not empty
    ...
    def init_question(self):
        ...
        self.btn_submit = tk.Button(self.frame_question, text="Submit", command=self.update_question, state=tk.DISABLED)
        # Bind key event to enable submit button when there is input '''
        self.entry_answer.bind("<KeyRelease>", self.validate_input)
...
# Bonus-1: Automatically calculate relevance
    def calculate_relevance(self):
        """
        Dynamically calculates relevance scores based on scene questions and apps.
        Apps that appear in more questions get higher relevance scores.
        """
        question_counts = {app: 0 for app in self.apps.keys()}
        for question in self.questions:
            if question["app"] in question_counts:
                question_counts[question["app"]] += 1
        
        max_count = max(question_counts.values(), default=1)
        self.relevance = {app: count / max_count for app, count in question_counts.items()}
